...
VRSA supports privately synchronizing your projects in [Google Drive](action://backup-google-drive) and [Dropbox](action://backup-dropbox).  
...
- VRSA receives a secure access token from them
- VRSA never sees your password or personal data
- No personal data or assets are stored on VRSA servers
...
Every time you click VRSA menu > Cloud Backup > (Select service), VRSA synchronizes all projects and samples with
your connected cloud service, uploading new and changed files and removing those that were deleted locally.

## How it works

During backup, VRSA uploads new and modified files and updates its index.json catalog for each domain (projects and
...
2. Click **Manage apps**
3. Find **VRSA** in the list
4. Choose one of the following:
    - **Disconnect from Drive** → removes VRSA’s access to your Drive
    - **Delete hidden app data** (if shown) → permanently deletes all files VRSA stored in your hidden appData space
...
2. Find **VRSA** in the list
3. Click **Disconnect**
4. When prompted, choose to also **delete all app folder data** if you want to remove files created by VRSA
